...
- **Sorted by event type.** A corporate inquiry goes to the **Corporate** tab, a
  wedding to **Wedding**, a debut or birthday to **Social**, and so on. Wording
  is matched, not just exact values: "Company Christmas Party" is corporate,
  "Church Wedding Reception" is a wedding, "18th Birthday" is social.
...
| `Corporate`, `Wedding`, `Social`, `Private Event` | The working queues, one per sales team |
...
| `_Settings` | Every knob: dedupe rules, reps for round-robin, notification addresses |
...
src/
  ...
  FairImport.html   The import dialog
...
headers, round-robin assignment, and webhook token rejection — asserting on
what actually lands in each tab.
