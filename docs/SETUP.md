...
3. Leave **Assigned Count** and **Last Assigned At** alone — that's the
   rotation's memory.
4. Check the presenter rows in `_Settings`:
   - `Presenters` ships as `AJ, Pam, Mhay, Vanessa` — the sequence written down
     the first column of a caller's tab, in rotation order.
   - `Presenters - Corporate` ships as `caller`, because corporate is called and
     presented by the same two people. A corporate lead in Shane's tab shows
     Shane as presenter.
